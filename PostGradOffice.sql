...
NonGucianStudent on delete cascade on update cascade,
...
if(@Gucian=1)
insert into GucianStudent(id,firstName,lastName,faculty,address,GPA,type) values(@id,@first_name,@last_name,@faculty,@address,@gpa,@type)
else
insert into NonGucianStudent(id,firstName,lastName,faculty,address,GPA,type) values(@id,@first_name,@last_name,@faculty,@address,@gpa,@type)
end
...
begin 
	select @id = id from PostGradUser P where P.email = @mail
...
@ThesisSerialNo int , @DefenseDate Datetime , @ExaminerName varchar(20),@ExaminerEmail varchar(20),@Password varchar(30),@National bit, @fieldOfWork varchar(20)
as
insert into PostGradUser values(@ExaminerEmail,@Password)
declare @id int
...
@studentID int, @firstName varchar(20), @lastName varchar(20), @password varchar(30), @email varchar(50), @address varchar(50), @type varchar(3)
as
...
@studentID int
as
select P.id as 'Payment Number', P.amount as 'Amount of Payment',P.fundPercentage as 'Percentage offund for payment', P.noOfInstallments as 'Number of installments',I.amount as 'Installment Amount',I.date as 'Installment date', I.done as 'Installment done or not'
...
@studentID int
as
select P.id as 'Payment Number', P.amount as 'Amount of Payment', P.fundPercentage as'Fund',P.noOfInstallments as 'Number of installments',I.amount as 'Installment amount',I.date as 'Installment date', I.done as 'Installment done or not'
...
create procedure findIdByMail @mail varchar(40), @id int output 
as	
	begin 
		select @id = id from PostGradUser P Where P.email = @mail
	end
...
create procedure getPersonalInfo @email varchar(40), @name varchar(40) output, @fieldOfWork varchar(40) output
as 
	begin 
		declare @id int ;
		select @id = id from postgraduser where email = @email
		select @name = name from Examiner where id = @id
		select @fieldOfWork = fieldOfWork from Examiner where id =  @id
	end
...
create procedure updateExaminer @email varchar(40), @name varchar(40), @fieldOfWork varchar(40), @succ bit output
as 
	begin 
		declare @id int 
		select @id = id from PostGradUser where email = @email
...
create procedure listAllExaminer @email varchar(30)
as 
	...
	begin
		...
		select T.serialNumber as 'SerialNo', T.title as 'Title', S.name AS 'Supervisor', GS.firstName + GS.lastName AS 'Student', T.defenseDate AS 'Date'
		from  ExaminerEvaluateDefense EED Inner Join GUCianStudentRegisterThesis GSRT ON EED.serialNo = GSRT.serial_no
										  Inner Join Thesis T ON T.serialNumber =EED.serialNo
										  Inner Join GucianStudent GS ON GS.id = GSRT.sid
										  Inner Join Supervisor S ON GSRT.supid = S.id
									
		where EED.examinerId = @id AND EED.date = T.defenseDate

	    UNION (
				select  T.serialNumber as 'SerialNo',T.title as 'Title', S.name AS 'Supervisor', NGS.firstName + NGS.lastName AS 'Student',  T.defenseDate AS 'Date'
				from  ExaminerEvaluateDefense EED Inner Join NonGUCianStudentRegisterThesis NGSRT ON EED.serialNo = NGSRT.serial_no
										  Inner Join Thesis T ON T.serialNumber =EED.serialNo
										  Inner Join NonGucianStudent NGS ON NGS.id = NGSRT.sid
										  Inner Join Supervisor S ON NGSRT.supid = S.id
				where EED.examinerId = @id AND EED.date = T.defenseDate
...
create proc addGradeWeb @serialNo int, @date datetime, @grade numeric(4,2)
as 
	begin 
		update defense
		set grade = @grade
		where defense.serialNumber = @serialNo AND defense.date = @date
		exec AddGrade @serialNo 
	end


go
create proc findThesisByName @subString varchar(40)
as 
	begin
		select * from Thesis 
		where Thesis.title like '%' + @subString + '%'
	end

go
select * from Thesis

exec findThesisByName 'n'
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--List Theses of a given student--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
--////////////------\\\\\\\\\\\\--
create proc ListTheses
@studentID int
as
declare @gucian bit
if(exists(
...
if(@gucian = '1')
begin
select * from Thesis T inner join GucianStudentRegisterThesis GT
on T.serialNumber = GT.serial_no
where GT.sid = @studentID
end
else
begin
select * from Thesis T inner join NonGUCianStudentRegisterThesis GT
on T.serialNumber = GT.serial_no
where GT.sid = @studentID
end
...
--////////////-------------------\\\\\\\\\\\\--
--////////////-------------------\\\\\\\\\\\\--
--List Courses with Grades of a given student--
--////////////-------------------\\\\\\\\\\\\--
--////////////-------------------\\\\\\\\\\\\--
--////////////-------------------\\\\\\\\\\\\--
--////////////-------------------\\\\\\\\\\\\--
...
create proc LisrCoursesNamesGrades
@studentID int
as
select C.code,C.creditHours,NGC.grade
from NonGucianStudentTakeCourse NGC inner join Course C
on NGC.cid = C.id
...
insert into Thesis (field,type,title,startDate,endDate,noOfExtensions) values('met','PhD','Neurel Networks','1/1/2019','1/1/2021',2)
select * from Thesis
insert into GUCianStudentRegisterThesis values(1,4,3);
insert into GUCianStudentRegisterThesis values(1,4,5);
insert into NonGUCianStudentRegisterThesis values(3,4,6);

--insert into GUCianProgressReport (sid,no,date,state,description,thesisSerialNumber,supid) values(1,1,'2/2/2019',5,'GOOD LIFE',1,3)
insert into Publication (title,dateOfPublication,place,accepted,host) values('love of the sea','3/3/2019','London',1,'Mosalah')
insert into ThesisHasPublication values (1,1)

exec AddCourse @courseCode='CSEN501',@creditHrs= 4,@fees= 3000
exec AddCourse @courseCode='Comm401',@creditHrs= 4,@fees= 2500
exec linkCourseStudent


insert into Thesis (field,type,title,startDate,endDate,noOfExtensions) values('iet','PhD','Neurel Networks II','1/1/2019','1/1/2021',2)


select * from GUCianStudentRegisterThesis
insert into GUCianStudentRegisterThesis values(2,3,1);
insert into Defense values(1,'1/1/2022','SPACE', null);
insert into ExaminerEvaluateDefense(serialNo, date, examinerId) values (1,'1/1/2022', 1)
